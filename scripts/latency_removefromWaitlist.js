...
module.exports = async function (callback) {
  try {
    ...
    const filePath = path.join(resultsDir, "latency_removeFromWaitlist.csv");
    ...
    for (let i = 1; i <= 20; i++) {
      ...
      // Precondition: add first
      await instance.addToWaitlist(user, {
        from: admin,
        gas: 300000
      });
      ...
      const tx = await instance.removeFromWaitlist(user, {
        from: admin,
        gas: 300000
      });
    ...
    console.log("Saved to latency_removeFromWaitlist.csv");
